package io.github.treesitter.jtreesitter.treesittersynovium;

import java.lang.foreign.*;

public final class TreeSitterTreeSitterSynovium {
    ...
    private static final TreeSitterTreeSitterSynovium INSTANCE = new TreeSitterTreeSitterSynovium();

    private final Arena arena = Arena.ofAuto();
    private volatile SymbolLookup lookup = null;

    private TreeSitterTreeSitterSynovium() {}
    ...
    public static MemorySegment language(SymbolLookup lookup) {
        return call(lookup, "tree_sitter_tree_sitter_synovium");
    }

    private SymbolLookup findLibrary() {
        try {
            var library = System.mapLibraryName("tree-sitter-tree-sitter-synovium");
            return SymbolLookup.libraryLookup(library, arena);
        } catch (IllegalArgumentException ex1) {
            try {
                System.loadLibrary("tree-sitter-tree-sitter-synovium");
